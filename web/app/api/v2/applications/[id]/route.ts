...
import { getApplicationV2 } from "../../_lib/queries";

type RouteContext = { params: Promise<{ id: string }> };

/**
 * GET /api/v2/applications/[id] — one application, scoped to the acting
 * user's workspace. An application id belonging to another workspace, or
...
export async function GET(request: Request, context: RouteContext) {
  return withReadOnlyV2Workspace(request, async (workspaceId) => {
    ...
    const data = await getApplicationV2(workspaceId, id);
