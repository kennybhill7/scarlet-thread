...
import { getMotifCandidateV2 } from "../../_lib/queries";

type RouteContext = { params: Promise<{ id: string }> };

/**
 * GET /api/v2/motifs/[id] — one motif candidate, scoped to the acting
 * user's workspace. A candidate id belonging to another workspace, or one
...
export async function GET(request: Request, context: RouteContext) {
  return withReadOnlyV2Workspace(request, async (workspaceId) => {
    ...
    const data = await getMotifCandidateV2(workspaceId, id);
