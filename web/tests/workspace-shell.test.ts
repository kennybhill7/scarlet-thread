...
// their own real contentModes ("context"/"theology"/"conviction"). This test
// is updated again by TEACHDRAFTPANE-001 (not a "gating" or "READGATE-001"
// test per acceptance criterion 6 — it is specifically the content-mode
// boundary each of these tasks' own job is to move) to also move Teach:
// Connect/Apply remain the only two still-unbuilt placeholders now.
test("computeWorkspaceSections: Connect/Apply are always contentMode 'placeholder', regardless of gate state -- Context/Theology/Conviction/Teach are real now", () => {
  ...
  const placeholderSteps: StudySessionStep[] = ["connect", "apply"];
...
// CLAIMPANES-001 / TEACHDRAFTPANE-001: only Connect/Apply remain "not built
// yet" placeholders now (see the contentMode test above); Context/Theology/
// Conviction get their own real-section coverage in tests/claim-panes.test.ts,
// and Teach gets its own real-section coverage in tests/teach-pane.test.ts.
test("RENDER: Connect/Apply each render an honest 'not built yet' placeholder, never fabricated content", () => {
  const html = render({ session: sampleSession() });
  const placeholderSteps: StudySessionStep[] = ["connect", "apply"];
  for (const step of placeholderSteps) {
  ...
  for (const step of ["context", "theology", "conviction", "teach"] as StudySessionStep[]) {
    assert.ok(
