...
**Stack:** Next.js 16.2 / React 19.2, Drizzle + Neon Postgres, NextAuth v5 (Google), IndexedDB offline vault, Node test runner (35 tests today) — unchanged **except** the vault's sync protocol: it runs client-clock last-write-wins today, and that is the one piece of "the stack" this plan does not keep, per tenet 4 below (retired in Phase 1, replaced by per-entity revisions + prose-conflict preservation, §3.4/§14).
...
**Route ownership for Teach (V1):** there is no standalone `teach/[id]/` route in V1. Teach-back is section 8 of `study/[sessionId]/` (§4) — it stays inside the Passage Workspace like every other step, reading and writing the same session's `TeachingDraft`. A dedicated `app/(app)/teach/` surface appears only in Phase 4 (§6) as an add-on **retrieval-review** mode (spaced re-teach prompts against drafts already created inside a session); it never becomes the place a first teach-back draft is written.
...
/** How a thread came to exist. Only `learner_promoted` carries the third-sighting
 *  requirement (§3.3 `study_thread_sightings`) — `starter` and `imported` threads
 *  may be linked on any single sighting because their warrant already exists. */
export type ThreadProvenance = "starter" | "imported" | "learner_promoted";
/** Curated-help reveal gates for the Passage Workspace (§4). Each value names the
 *  persisted artifact whose existence unlocks the matching curated help; a gate is
 *  computed by querying for that artifact, never read off `currentStep`. */
export type RevealAfter =
  | "read" | "observation" | "interpretation_attempt"
  | "comparison_attempt" | "theology_claim" | "application_attempt"
  | "teaching_draft";
```
...
- `study_sessions` — id, workspaceId, canonical range, mode, workflowState (active | closed | archived — the session's own lifecycle, set directly when the learner closes/reopens), threadResolution (unexamined | provisional | linked — **derived**, never hand-set: computed from the session's `study_thread_sightings` rows every time one changes, see below), optional pinned catalog/curriculum release, readGateAt, currentStep. `currentStep` is a navigation convenience (which accordion section reopens) only — no curated-help reveal gate anywhere in §4 reads it; every gate reads a persisted artifact instead. The workspace routes by **sessionId**, not book/chapter: multiple studies of the same passage, resumability, and revision history all need it.
- `study_claims` — id, workspaceId, sessionId, kind (ClaimKind), epistemicBasis (EpistemicBasis), body, passage (CanonicalRange), confidence (ClaimConfidence), provenance (ClaimProvenance), doctrineStatus (theology claims only), viewpoint (nullable), status (draft | revisited | confirmed | needs_revision). Indexes on (workspaceId, passage), (workspaceId, kind). The learner's **first observation is preserved permanently** via **append-only `artifact_revisions` rows** — an integer revision counter alone is not the guarantee; every edit inserts the prior body as an immutable row. `conviction` claims are private-by-design, and that privacy is an **enforced, tested policy across five surfaces**: analytics/learning measures, server logs/telemetry, sharing, AI retrieval (Phase 6 corpus assembly must filter them), and teaching promotion — each exclusion has its own test.
- `claim_evidence` — claimId, workspaceId, kind ("passage" | "citation" | "connection" | "claim"), ref — where "citation" is a first-class `citationId` into the sources/citations model (§3.3 curated), not a pasted string. One claim, many evidence rows, and the pair is an **atomic aggregate**: a claim insert/update and its evidence rows commit in one transaction (or one sync `mutationGroupId`, §3.4) — a claim is never accepted with only some of its evidence rows landed. An `interpretation` with zero evidence rows renders with a visible **"unsupported"** badge — the structural fix for audit gap #1.
- `motif_candidates` + `motif_sightings` — the radar and sightings 1–2 live here (label, normalized key, exact range, status), **before** any thread exists. Dismissal never deletes the underlying observation.
- `study_thread_sightings` — id, workspaceId, sessionId, threadSlug (nullable), motifCandidateId (nullable — set while still provisional, before any thread exists), passage-unit key/canonical range, sightingSource (starter | imported | learner_observed). One row per distinct passage scope the session touched. A `study_sessions.threadResolution` of `linked` means at least one active row here has a non-null `threadSlug`. **Linking an existing starter or imported thread needs only one sighting** — it already carries its own reviewed warrant. **Creating a brand-new thread (`ThreadProvenance = "learner_promoted"`)** is the only path that must satisfy all three: (1) three `study_thread_sightings` rows for the same candidate, (2) each in a distinct passage unit (three visits to one passage do not count), and (3) an explicit learner confirmation action — promotion is never silent. Promotion is one atomic transaction: it creates the thread row (`provenance = learner_promoted`) and rewrites exactly those three sighting rows' `motifCandidateId` → `threadSlug` together, or none of them (matching the third-sighting rule already in `THEOLOGY_MASTER_BUILD_PLAN.md` Law 3/4).
- `comparison_attempts` — id, workspaceId, sessionId, claimId (nullable, the `user_connections` row produced if any), both passage ranges, outcome (warranted | no_warrant_yet), rationale, timestamps. `no_warrant_yet` lives **only** here, scoped to one attempt — it is not a `study_sessions.threadResolution` value, so one honestly unwarranted comparison never overwrites an already-`linked` session. An honest, substantive `no_warrant_yet` rationale satisfies the Connect attempt-gate exactly like a warranted one (§4): the gate checks that a genuine attempt was persisted, not that it produced a specific claim.
- `user_connections` — id, workspaceId, fromRange, toRange, type (ConnectionType), evidenceLabel (EvidenceLabel, with the §3.2 personal_resonance/devotional constraints), rationale (required, min 20 chars), threadSlug (nullable), status (draft | confirmed | revisited). Unique on (workspaceId, fromRange, toRange, type). **Learner-authored only**: reviewed edges live in curated `graph_edges` (below) and radar output lives in `motif_candidates` — the three provenances never share a table. Every connection renders both passages side by side; the evidence label keeps "these verses sound similar" from quietly becoming "the Bible teaches this."
...
- `teaching_drafts` + `teaching_sections` — the master plan §12.3 shape: draft holds id, workspaceId, sessionId, format (60-second explanation | 5-minute table talk | 15-minute small-group lesson | 30-minute teaching), title, bigIdea, audience, gospelConnection, status, revision, deliveredAt (nullable); ordered sections carry kind (outline | context | connection | theology | illustration | objection | application | not_justified | discussion | prayer), sortOrder, body. A `connection` section accepts a reasoned `no_warrant_yet` in place of an edge. Same draft/finalize rule. A draft and its ordered sections are the same atomic aggregate as `study_claims`/`claim_evidence` above — sections commit with their draft or not at all, preserving `sortOrder` gaps-free. Software flags missing evidence or structure; it never scores theological correctness. Review state in the solo MVP is **Draft complete / Rehearsed / Self-reviewed** (master plan §22); nothing here becomes **Human-reviewed** without an actual independent qualified reviewer — software cannot self-certify that tier.

**v1 thread invariant stays exactly as it is.** `lib/api/entries.ts` requires ≥1 thread per `Entry` and `0003_enforce_active_thread_links.sql` enforces it at the database layer — that requirement, `syncEntrySchema` in `lib/api/sync.ts`, and the 0003 triggers are **not touched by this plan**. The apparent conflict with the third-sighting rule (sightings 1–2 of a new motif have no thread yet) does not actually reach v1: `study_sessions`, `study_claims`, `motif_candidates`/`motif_sightings`, and `study_thread_sightings` are new v2 tables with no per-row thread requirement of their own, so a v2 draft can exist unlinked without ever creating a v1 `Entry` row that violates the trigger. The only bridge between the two models is the existing, optional `legacy_entry_id` (master plan §12.2): promoting a legacy `Entry` into a `StudyClaim` copies a record that, by v1's own rule, already carries ≥1 thread — promotion never needs to create or import an unthreaded `Entry`. The v2-only rule — a session may not claim `threadResolution = linked` without a resolved `study_thread_sightings` row — is enforced entirely on v2 tables (§3.3 above).
...
- Extend `SyncEntity` with the six new entities — `"session" | "claim" | "motif" | "connection" | "application" | "teachingDraft"` — and add the matching arrays to `SyncResponse` (additive; related offline creations sync as a bounded atomic group per master plan §14). `study_thread_sightings` and `comparison_attempts` rows are not separate top-level sync entities: each syncs inside its parent session's/claim's `mutationGroupId`, on the same atomic-aggregate rule as `claim_evidence` and `teaching_sections` above — the aggregate accepts or rejects as one unit, never partially.
- **All browser writes go through sync push** (tenet 7). New v2 routes are **read-only** resource endpoints (`/api/v2/workspaces/[id]/claims`, `/connections`, `/applications`, `/teaching-drafts`); conflict resolution invokes the same authoritative revision/idempotency transaction as sync. No independent REST mutations.
...
One route: `app/(app)/study/[sessionId]/page.tsx` — a session pins its canonical passage range and (when curriculum-attached) a catalog release. Eight sections in one accordion; `currentStep` only remembers which section reopens on return. It is **not** the reveal mechanism.

**Gating principle (revised per master plan §30):** the strict gate is *learner attempt before curated help* — you must try before the reviewed material reveals. Every "Curated help unlocks after" cell in the table below is one `RevealAfter` value (§3.2) evaluated by checking for the persisted artifact it names (a saved `StudyClaim`, a `comparison_attempts` row, a finalized `Application`, …), fetched fresh on each render — never `currentStep`, and never an in-memory/client-only boolean, both of which can desync from what actually saved across a reload or a second device. A gate only asks whether a genuine attempt was persisted, not whether it reached a particular "correct" conclusion: an honest, substantive `no_warrant_yet` on a `comparison_attempts` row satisfies the Connect gate exactly as a warranted connection would, so fabricating a claim to force a reveal is never necessary. Producing an artifact is **not** forced at every step: Theology does not require a Connection first (the Connect step may honestly close with `no_warrant_yet` instead), because forcing a connection trains users to invent one.
