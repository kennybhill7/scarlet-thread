...
> **Unenumerated status fields (recorded 2026-08-18, found by STUDYV2-001).** Several columns are named in both plans but their allowed values are enumerated in neither: `applications.application_class`, `applications.promise_scope`, and the `status` fields on `applications`, `teaching_drafts`, `motif_candidates` and `motif_sightings`. Until a reviewed decision enumerates them they are typed as free `string` in `lib/contracts/study-v2.ts` and must be plain text columns in the schema — **do not invent enum values for them**, because a guessed enum becomes a destructive migration once the real vocabulary is decided. (Drizzle; user tables workspace-scoped w/ soft delete, curated tables are read-only release indexes)
...
- `teaching_drafts` + `teaching_sections` — the master plan §12.3 columns verbatim: draft holds id, workspaceId, sessionId, title, bigIdea, audience, durationMinutes, gospelConnection, status, revision, timestamps (the four vision formats — 60-second, 5-minute, 15-minute, 30-minute — are UI presets that set `durationMinutes`, not a column); ordered sections carry kind (outline | context | connection | theology | illustration | objection | application | not_justified | discussion | prayer), sortOrder, body. A `connection` section accepts a reasoned `no_warrant_yet` in place of an edge (master §6.6 says the same). Same draft/finalize rule. Software flags missing evidence or structure; it never scores theological correctness.
...
- **All browser writes go through sync push** (tenet 7). New v2 routes are **read-only** resource endpoints (`/api/v2/workspaces/[id]/claims`, `/connections`, `/applications`, `/teach`); conflict resolution invokes the same authoritative revision/idempotency transaction as sync. No independent REST mutations. **One narrow, named exception (RADARUI-001):** motif-candidate **confirm/dismiss** (`POST /api/motifs/[id]`) is a server-side transition, not a client-authored edit. Sync push carries client-held field values under optimistic concurrency; confirm/dismiss carry no payload at all — the learner picks one of exactly two transitions the server defined on a candidate the server itself computed. Promotion is also one atomic batch spanning the candidate, all of its sightings, and a **v1 `threads` row**, which the v2 sync wire format has no entity for. Routing it through sync push would either lose that atomicity or rebuild this same privileged logic inside the sync planner — the second write path tenet 7 exists to prevent, just relocated. The full reasoning lives in the route header. **Consequence to hold onto:** if v2 sync push ever goes live, it must NOT be able to set `motif_candidates.status` independently, or the learner-confirmation and three-live-sightings guarantees are bypassable (see MOTIFSTATUS-001).
